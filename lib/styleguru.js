...
exports.parse = function(source, options)
{
	...
	nodeCheckers = {
		...
		SequenceExpression: function(node, depth)
		{
			var errors = [];

			node.expressions.forEach(function(expression)
			{
				errors = errors.concat(checkNode(expression, depth));
			});
		...
		VariableDeclarator: function(node, depth)
		{
			...
			return errors.concat(checkNode(node.id, depth), checkNode(node.init, depth));
		},
