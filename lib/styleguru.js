	...
	nodeCheckers = {
		...
		BlockStatement: function(node, depth)
		{
			/*
				{(<ws><statement)*<ws>}
			*/
			var start = node.range[0] + 1, i = 0, l = node.body.length, whitespace;

			if(l === 0)
			{
				whitespace = source.slice(start, node.range[1] - 1);
				// Whitespace for an empty block should be \n(\t{depth})
				expect("whitespace", node, "\n\n?" + repeat("\t", depth), whitespace, start);
				return;
			}
			...
			for(; i < l; i++)
			{
				...
				expect("whitespace", node.body[i], "\n\n?" + repeat("\t", depth + 1), whitespace,
					start);
				start = node.body[i].range[1];
			}

			whitespace = source.slice(node.body[l - 1].range[1], node.range[1] - 1);
		},
		...
		ForInStatement: function(node, depth)
		{
			...
			*/
			var a = node.range[0] + "for".length,
				b = a + source.slice(a, node.left.range[0]).indexOf("("),
				c = node.left.range[0],
				d = node.left.range[1],
				e = d + source.slice(d, node.right.range[0]).indexOf("in"),
				f = e + "in".length,
				g = node.right.range[0],
				h = node.right.range[1],
				i = h + source.slice(h, node.body.range[0]).indexOf(")"),
				j = node.body.range[0];

			var ws1 = source.slice(a, b),
				ws2 = source.slice(b + 1, c),
				ws3 = source.slice(d, e),
				ws4 = source.slice(f, g),
				ws5 = source.slice(h, i),
				ws6 = source.slice(i + 1, j);
		...
		ForStatement: function(node, depth)
		{
			...
			*/
			// Ugh, init, test and update are all optional
			var a = node.range[0] + "for".length,
				l = node.body.range[0],
				b = a + source.slice(a, l).indexOf("("),
				c = node.init ? node.init.range[0] : b + 1,
				d = node.init ? node.init.range[1] : c,
				e = d + source.slice(d, l).indexOf(";"),
				f = node.test ? node.test.range[0] : e + 1,
				g = node.test ? node.test.range[1] : f,
				h = g + source.slice(g, l).indexOf(";"),
				i = node.update ? node.update.range[0] : h + 1,
				j = node.update ? node.update.range[1] : i,
				k = j + source.slice(j, l).indexOf(")");

			var ws1 = source.slice(a, b),
				ws2 = source.slice(b + 1, c),
				ws3 = source.slice(d, e),
				ws4 = source.slice(e + 1, f),
				ws5 = source.slice(g, h),
				ws6 = source.slice(h + 1, i),
				ws7 = source.slice(j, k),
				ws8 = source.slice(k + 1, l);
		...
		FunctionDeclaration: function(node, depth)
		{
			/*
				|        a    b    c    d     e       f    g     h
				function[<ws1><id>]<ws2>(<ws3><params><ws4>)<ws5><body>
			*/

			// Yuck!
			var a = node.range[0] + "function".length,
				b = a,
				c = a,
				d, e, f, g,
				h = node.body.range[0];
			...
			{
				e = node.params[0].range[0];
				d = c + source.slice(c, e).indexOf("(");
				f = node.params[node.params.length - 1].range[1];
				g = f + source.slice(f, node.body.range[0]).indexOf(")");
			}
			else
			{
				d = c + source.slice(c, h).indexOf("(");
				e = f = g = d + source.slice(d, h).indexOf(")");
			}

			var ws2 = source.slice(c, d),
				ws3 = source.slice(d + 1, e),
				ws4 = source.slice(f, g),
				ws5 = source.slice(g + 1, h);
